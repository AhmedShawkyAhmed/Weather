...
extension Color {
    static let backgroundGrad = LinearGradient(gradient: Gradient(colors: [Color("Background 1"), Color("Background 2")]), startPoint: .topLeading, endPoint: .bottomTrailing)
    ...
    static let tabBack = Color("Tab Bar Border").opacity(0.5)
    static let forecast = Color("Forecast Card Background")
    static let probabilit = Color("Probability Text")
}
...
extension View {
    func innerShadow<S: Shape, SS: ShapeStyle>(shape: S, color: SS, lineWidth: CGFloat = 1, offsetX: CGFloat = 0, offsetY: CGFloat = 0, blur: CGFloat = 4, blendModel: BlendMode = .normal, opacity: Double = 1) -> some View {
        return self
            .overlay {
            shape
